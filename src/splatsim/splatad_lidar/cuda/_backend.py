...
try:
    ...
    from splatsim.splatad_lidar import csrc as _C
except ImportError:
    # if failed, try with JIT compilation
    if cuda_toolkit_available():
        name = "splatad_lidar_cuda"
        ...
        if os.path.exists(os.path.join(build_dir, "splatad_lidar_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "splatad_lidar_cuda.lib")
        ):
